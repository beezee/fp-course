...
-- Empty
appendOptional :: (Optional a) -> (Optional (List a)) -> (Optional (List a))
appendOptional Empty _ = Empty
appendOptional _ Empty = Empty
appendOptional (Full a) (Full as) = Full (a:.as)

seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight appendOptional (Full Nil)
...
-- >>> lengthGT4 infinity
-- True
lengthGT4Rec :: Integer -> List a -> Bool
lengthGT4Rec n Nil = n > 4
lengthGT4Rec n (_:.t)
  | (n > 4) = True
  | otherwise = lengthGT4Rec (n+1) t

lengthGT4 ::
  List a
  -> Bool
lengthGT4 as = lengthGT4Rec 0 as
