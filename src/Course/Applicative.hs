...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) fs as = flatten $ map (\f -> map (\a -> f a) as) fs
...
lift2 ::
  ...
  -> f b
  -> f c
lift2 abc fa fb = (<*>) (lift1 abc fa) fb
...
sequence (h :. Nil) = lift1 (\a -> a :. Nil) h
sequence (h :. t) = lift2 (++) (lift1 (\x -> x :. Nil)  h) (sequence t)
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering p as = (((<$>) snd) . (filter fst)) <$> fbas
  where fbas = sequence $ (\a -> (\x -> (x, a)) <$> (p a)) <$> as
