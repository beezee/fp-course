...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f sfa = StateT ((<$>) (\t -> ((f $ fst t), (snd t))) . runStateT sfa)
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    ...
    -> StateT s f b
  (<*>) sfab sfa =
    StateT (\s -> (runStateT sfab s) >>=
      (\t -> runStateT ((fst t) <$> sfa) $ snd t))
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' sa s = let (ExactlyOne t) = runStateT sa s in t
...
instance Functor f => Functor (OptionalT f) where
  (<$>) ab (OptionalT fa) = OptionalT $ (\oa -> case oa of
    (Full a) -> Full $ ab a
    Empty -> Empty) <$> fa
...
instance Monad f => Monad (OptionalT f) where
  (=<<) aotb (OptionalT foa) = OptionalT $
    foa >>= onFull (runOptionalT . aotb)
